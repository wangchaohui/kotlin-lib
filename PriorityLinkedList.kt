...
class PriorityLinkedList<T : PriorityLinkedList.Node<T>>(list: List<T>, comparator: Comparator<T>) {
    interface Node<N> : BinaryHeap.Item {
        var l: N?
        var r: N?
    }

    private val heap = BinaryHeap(comparator)

    init {
        list.forEachIndexed { i, node ->
            node.l = list.getOrNull(i - 1)
            node.r = list.getOrNull(i + 1)
        }
    ...
    fun updateAlreadyPoped(node: T) = node.apply {
        l?.r = r
        l?.let(heap::modify)
        r?.l = l
        r?.let(heap::modify)
